...
import pickle # ⚠️ Hardcoded Secret — API_KEY should not be hardcoded.
...
def get_user_by_name_vuln(name):
    ...
    return cursor.fetchall() # ⚠️ Command Injection — User input 'username' is directly concatenated into the shell command.

def list_user_files_vuln(username):
    os.system("ls /home/" + username + " > /tmp/out.txt")

app = Flask(__name__)
 # ⚠️ Insecure File Upload — Uploaded files are saved directly using the original filename.
