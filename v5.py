...
import pickle # ⚠️ Exposure of Sensitive Information — API key should not be hardcoded.

API_KEY = "SOME_SUPER_SECRET_KEY_12345"
 # ⚠️ SQL Injection — User input 'name' is directly concatenated into the SQL query.
def get_user_by_name_vuln(name):
    conn = sqlite3.connect("example.db")
    cursor = conn.cursor()
    query = "SELECT id, username FROM users WHERE username = '" + name + "';"
    cursor.execute(query)
    return cursor.fetchall() # ⚠️ Command Injection — User input 'username' is directly concatenated into the command.
