...
@app.route("/runcmd", methods=["POST"]) # ⚠️ Command Injection — User input is directly passed to subprocess.getoutput without validation. # ⚠️ Command Injection — Potentially allows execution of arbitrary commands.
...
# User input is directly passed to pickle.loads without validation. # ⚠️ Code Injection — User input is directly passed to eval without any sanitization. # ⚠️ Code Injection — Using eval on untrusted data is a known security risk.
...
@app.route("/unpickle", methods=["POST"]) # ⚠️ Code Execution via Pickle — User input is directly passed to pickle.loads without validation. # ⚠️ Code Execution via Pickle — Using pickle.loads() on untrusted data can allow attackers to execute arbitrary code.
