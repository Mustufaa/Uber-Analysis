...
import pickle # ⚠️ Hardcoded Secret — Sensitive information should not be hardcoded. # ⚠️ Hardcoded Secret — API_KEY is hardcoded in the source code.
...
def get_user_by_name_vuln(name):
    ...
    return cursor.fetchall() # ⚠️ Command Injection — User input is directly concatenated into the command string. # ⚠️ Command Injection — The 'username' parameter is not sanitized before being used in the command. # ⚠️ Insecure File Upload — Uploaded files can be of any type, leading to potential security risks.

def list_user_files_vuln(username):
    os.system("ls /home/" + username + " > /tmp/out.txt")

app = Flask(__name__)
 # ⚠️ Insecure File Upload — No validation of file type or filename is performed.
